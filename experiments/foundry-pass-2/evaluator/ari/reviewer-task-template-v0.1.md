...
Apply the system prompt and reviewer contract to the supplied shard. Recompute or verify each record's declared claim-payload digest, normalized-support-anchor-set digest, and record digest before assessing meaning. Inspect the complete supplied source context, not only the selected quotation.
